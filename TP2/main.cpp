...
bool liste_reserv(std::vector<Reservation> &list, Hotel &h,
                  std::vector<Client> &list_client, int numreserv) {
  // permet de creer la liste de toutes les reservations
  int day, month, year;
  int rsvNbr;
  ...
  if (i != -1) {        // si une chambre est dispo, cree la reservation
    ...
    int rsvNbr = entry.getNumReserv();
    std::cout << "Numero de reservation :" << rsvNbr << std::endl;
    return true;
  } else {
...
int main() {
  ...
  bool valide;
  valide = liste_reserv(list_reserv, h, list, numreserv);
  if (valide == true) {
    numreserv++;
  }
  valide = liste_reserv(list_reserv, h, list, numreserv);
